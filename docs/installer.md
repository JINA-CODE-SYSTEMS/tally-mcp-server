...
1. Unpacks `dist/`, `scripts/`, prebuilt `TallyUI.dll`, portable Node.js, and `nssm.exe` to `C:\Program Files\TallyMCP\`.
2. Runs a wizard that collects:
   ...
   - Public domain (blank = localhost-only)
...
preserved on uninstall (operators may have rotated secrets) — delete by hand
for a full wipe.
