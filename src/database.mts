...
// Disable external file access to prevent DuckDB file-system escape attacks
await conn.run("SET enable_external_access = false;");

// SQL safety: only allow SELECT statements, block dangerous keywords and file-access functions
const SQL_BLOCKED_PATTERN = /\b(INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|TRUNCATE|REPLACE|MERGE|COPY|ATTACH|DETACH|INSTALL|LOAD|EXPORT|IMPORT|CALL|EXECUTE|EXEC|PRAGMA|SET|GRANT|REVOKE)\b/i;
const SQL_BLOCKED_FUNCTIONS = /\b(read_csv_auto|read_csv|read_parquet|read_json|read_json_auto|read_text|read_blob|write_csv|write_parquet|httpfs|http_get|http_post|glob|system|getenv)\b/i;

export function validateSQL(sql: string): string | null {
    const trimmed = sql.trim().replace(/;+$/, '').trim();
    if (!trimmed) return 'Empty SQL query';
    ...
    // Block dangerous statements that could be embedded (e.g., in subqueries or unions)
    if (SQL_BLOCKED_PATTERN.test(trimmed)) {
        const match = trimmed.match(SQL_BLOCKED_PATTERN);
        return `Blocked SQL keyword: ${match?.[0]?.toUpperCase()}`;
    }
    
    // Block file-access and system functions
    if (SQL_BLOCKED_FUNCTIONS.test(trimmed)) {
        const match = trimmed.match(SQL_BLOCKED_FUNCTIONS);
        return `Blocked function: ${match?.[0]}`;
    }
