...
| `get-context` | — | `status` fields **plus** `requirements` — the external preconditions (Tally XML server, GUI agent, credentials, edition). Call this first on a fresh session to learn what must be running before loading a company. |
